...
public class InspectionApiController {
    ...
    @GetMapping("/me")
    public ResponseEntity<CommonResponse<GetMyInspectionsResponse>> getMyInspections(
    ...
    public ResponseEntity<CommonResponse<GetInspectionResponse>> getInspection(@PathVariable UUID inspectionId) {
        return ResponseEntity.ok(CommonResponse.of(HttpStatus.OK.value(), GetInspectionResponse.from(getInspectionUseCase.getInspection(inspectionId))));
    }
}
